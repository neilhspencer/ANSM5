...
mc.ci <-
  function(x, y = NULL, CI.width = 0.95, nsims.mc = 10000, seed = NULL) {
    ...
    if (is.null(y)){ #for one sample
      ...
      mediandiff.vec <- rep(NA, nsims.mc)
      nx <- length(x)
      ny <- length(y)
      if (!is.null(seed)){set.seed(seed)}
      for (i in 1:nsims.mc){
      ...
      mc.ci.lower <- quantile(mediandiff.vec, (1 - CI.width) / 2)
      mc.ci.upper <- quantile(mediandiff.vec, 1 - (1 - CI.width) / 2)
    }
