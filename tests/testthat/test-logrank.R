test_that("Example 9.6", {
  ...
  expect_equal(logrank(ch9$samplesAB.survtime, ch9$samplesAB.censor,
                       ...
                       nsims.mc = 100000)$pval.mc, 0.34223)
})
...
test_that("Example 9.7", {
  ...
  tmp <- logrank(ch9$samplesXYZ.survtime, ch9$samplesXYZ.censor, ch9$samplesXYZ,
                 max.exact.perms = 1, seed = 1)
  expect_equal(tmp$pval.mc.stat, 1.3702381)
  expect_equal(tmp$pval.mc, 0.305)
...
test_that("Exercise 9.6", {
  tmp <- logrank(c(ch9$regimeA.survtime, ch9$regimeB.survtime),
                 c(ch9$regimeA.censor, ch9$regimeB.censor),
                 factor(c(rep("RegimeA", 12), rep("RegimeB", 13))),
                 score.censored = FALSE, max.exact.perms = 10000000)
  expect_equal(tmp$pval.exact.stat, 2.3375973)
  expect_equal(tmp$pval.exact, 0.24978463)
  #MC
  tmp <- logrank(c(ch9$regimeA.survtime, ch9$regimeB.survtime),
                 c(ch9$regimeA.censor, ch9$regimeB.censor),
                 factor(c(rep("RegimeA", 12), rep("RegimeB", 13))),
                 score.censored = FALSE, seed = 1)
  expect_equal(tmp$pval.mc.stat, 2.3375973)
  expect_equal(tmp$pval.mc, 0.2512)
