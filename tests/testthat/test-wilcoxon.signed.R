test_that("exact p-val works", {
  ...
  expect_equal(wilcoxon.signed.rank.test(withties, 6, "greater")$pval.exact.stat, 5)
  expect_equal(wilcoxon.signed.rank.test(withties, 6, "greater")$pval.exact, 0.07812)
  expect_equal(wilcoxon.signed.rank.test(ages, 30, "greater")$pval.exact.stat, 5)
  expect_equal(wilcoxon.signed.rank.test(ages, 30, "greater")$pval.exact, 0.07812)
})
