test_that("Example 4.2", {
  expect_equal(suppressWarnings(
    ks.test.ANSM(ch4$breaks, "punif", min = 0, max = 6,
...
test_that("Exercise 4.2", {
  expect_equal(suppressWarnings(
    ks.test.ANSM(ch4$waiting.time, "pexp", rate = 1 / 20)$pval.exact),
    0.16521394)
})

test_that("Exercise 4.3", {
  expect_equal(ks.test.ANSM(ch4$visiting.supporters, "pexp",
                            rate = 2600)$pval.exact, 0)
  expect_equal(ks.test.ANSM(ch4$visiting.supporters, "punif", min = 1100,
                            max = 7700)$pval.exact, 0.000227738766)
})

test_that("Example 6.15", {
  expect_equal(ks.test.ANSM(ch6$salivaF, ch6$salivaM,
...
test_that("Exercise 6.13", {
  expect_equal(ks.test.ANSM(ch6$time.withoutLD,
                            ch6$time.withLD)$pval.exact, 0.055944056)
  expect_equal(ks.test.ANSM(ch6$time.withoutLD, ch6$time.withLD,
                            alternative = "greater")$pval.exact, 0.027972028)
})

test_that("Exercise 6.15", {
  expect_equal(ks.test.ANSM(ch6$doseI, ch6$doseII)$pval.exact,
               0.033566434)
})
